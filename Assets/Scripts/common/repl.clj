(ns common.repl
  ...
  (:import ArcadiaBehaviour ArcadiaState
           ...
           [UnityEngine GameObject Resources Camera Vector3 LayerMask]
           [UnityEngine.SceneManagement SceneManager]
           [UnityEditor AssetDatabase PrefabUtility EditorUtility])
  (:require [arcadia.internal.protocols :refer [snapshotable?]]
            [arcadia.core :as ac]
            ;[arcadia.sugar :as as]
...
(defn prefab? [obj]
  (if (null->nil obj)
    ...
    ; (or (PrefabUtility/IsPartOfPrefabAsset obj)
    ;     (PrefabUtility/IsPartOfPrefabInstance obj))))

(defn gobj-or-cmpt? [obj]
  (or (instance? UnityEngine.GameObject obj)
      (instance? UnityEngine.Component obj)))
  ...
  ([id]
   (cond
     ...
     (and (instance? UnityEngine.Object id) 
          (not (zero? (int (PrefabUtility/GetPrefabAssetType id)))))
...
(defn resources "equivalent to resource, but returns all hits"
  ...
  ([id -utype]
   (cond 
     (nil? id) nil
     (gobj-or-cmpt? -utype)
     (distinct (sequence (comp (map #(resource % GameObject)) (filter #(o= id %))) (descendents (gobj -utype))))
     (instance? UnityEngine.Object -utype)
     (if-let [one (resource -utype (type -utype))] (list one))
     (and (gobj-or-cmpt? id) (not (prefab? id)))
  ...
  ([id utype]
   (as-> #(o= id %) pred
     (if-let [id (resource id GameObject)]
       (if (not (prefab? id))
         #(and (gobj-or-cmpt? id) ;i.e. pls don't break
               (= (gobj id) 
                  (gobj %)))
         pred)
       pred)
...
(defn clone! "instatiates trying to keep prefab connection.\n  if passed two arguments, will clone second into first."
  ...
  ([id utype]
   (cond
     ...
     (let [id (if (gobj-or-cmpt? id) id (clone! id GameObject))
           typ (type utype)
     ...
     (let [id (if (instance? (type utype) id) id (clone! id (type utype)))]
       ...
       (if (gobj-or-cmpt? utype)
         (reduce clone! id (cmpts (gobj utype) UnityEngine.Component)))
       id)
     :else
     (let [id (if (gobj-or-cmpt? id) id (resource id GameObject))]
       (when-let [x (state (gobj id) utype)]
         (if (snapshotable? x)
           (mutable (snapshot x))
           x))))))
...
(defn add-layer! "adds GameObject resources found with id to layer, creating the layer if nonexistent."
  [id layer-name]
  (if (or (TagsAndLayers/LayerExists layer-name) 
          (TagsAndLayers/AddLayer layer-name))
    (run! #(->> (.value (LayerMask/GetMask layer-name))
                (bit-or (.layer %))
                (set! (.layer %)))
          (resources id GameObject))))
