...
;;TEMP? arcadia-unity #273 (merged, then removed at beta)
...
;TODO
;I should move roles+ from complete to init, after fab @todo
   ...
   (cond
     ...
     (let [node (first x)
       ...
       (fn
         ...
         ([acc in] (root acc in))))
     (map? x)
     (let [node (::init (meta x))
           root (trampoline node rf)]
       (fn
         ([] 
          (let [[h & t] (root)]
            (cons (vary-meta x assoc ::init h) 
                  t)))
         ([acc] (root acc))
         ([acc in] (root acc in))))
     (nil? x)
...
;concentrate all the kinkness
(defn fab "clones from id's resource, if found.\n  when passed a role will look for a ::init meta.\n attempts to tag by id."
  [id*]
  (as-> id* id 
    (if (map? id) 
      (::init (meta id))
    ...
    (if-let [src (resource id GameObject)]
      (let [old (.activeSelf src)]
        (.SetActive src false) ;avoid start/awake (questionable)
        (let [gob (clone! src)]
          (.SetActive src old)
          (try
